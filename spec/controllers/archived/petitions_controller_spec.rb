    ...
    context "when a state param is provided" do
      context "but it is not a public facet from the locale file" do
        it "redirects to itself with state=all" do
          ...
          expect(response).to redirect_to "https://test.epetitions.website:3443/archived/petitions?state=all"
        end

        it "preserves other params when it redirects" do
          get :index, q: "what is clocks", state: "awaiting_monkey"
          expect(response).to redirect_to "https://test.epetitions.website:3443/archived/petitions?q=what+is+clocks&state=all"
        end
      end
  ...
  describe "GET #show" do
    ...
    context "when the petition is archived but the parliament is not" do
      ...
      it "redirects to the current petition" do
        get :show, id: petition.id
        expect(response).to redirect_to "https://test.epetitions.website:3443/petitions/#{petition.id}"
      end
    end
  end
end
